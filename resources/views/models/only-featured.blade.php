                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive col-lg-12" id="tableid">
                                <table class="table table-striped table-hover department-table">
                                    ...
                                    @if(!empty($featured) && count($featured)>0)
                                        @foreach($featured as $f)
                                            <tr>
                                                <th scope="row">{{ ($featured->currentpage()-1) * $featured->perpage() + $loop->index + 1 }}</th>
                                                <td>
                                                    <a href="{{route('model.show', $f->model_id)}}">{{$f->model_name}}</a>
                                                </td>
                                                <td>
                                                    {{$f->launch_date}}
                                                </td>
                                                <td>
                                                    <a href="{{route('model.show', $f->model_id)}}"
                                                       class="btn btn-sm btn-success"><i class="far fa-eye" data-toggle="tooltip" data-placement="top" title="View"></i></a>
                                                    <a href="{{route('model.edit', $f->model_id)}}"
                                                       class="btn btn-sm btn-primary">
                                                        <i class="far fa-edit" data-toggle="tooltip" data-placement="top"
                                                           title="Edit"></i>
                                                    </a>
                                                    <a href="{{route('model.destroy', $f->model_id)}}" data-uuid="{{$f->model_id}}"
                                                       data-name="{{$f->model_name}}"
                                ...
                                <div class="text-center">{{$featured->links()}}</div>
                            </div>
                        </div>
                    </div>
                </div>
